...
{
	$operator_id = filter_input(INPUT_POST, "operator");
	$operator = Users::with_id($operator_id);
	if(!$operator)
	{
		$_SESSION["error_msg"] = "addrfid.php: unable to create operator for $operator_id";
		header("Location:./addrfid.php");
		exit();
	}

	$rfid_no = filter_input(INPUT_POST, "rfid");
	if($operator->rfid_no)
	{
		if(filter_input(INPUT_POST, "override") != "yes")
		{
			__header__echo_go_modal("You must indicate that you want to overwrite <br>the exisiting RFID number.", "Error");
			exit();
		}

		if(!$user->update_rfid($rfid_no, $operator))
		{
			$_SESSION["error_msg"] = "Failed to update RFID";
			header("Location:./addrfid.php");
		}
		else 
		{
			$_SESSION['success_msg'] = "ID:$operator_id's RFID was updated.";
			$operator_id = $rfid_no = "";
	...
	{
		if(!$user->new_rfid($rfid_no, $operator))
		{
			$_SESSION["error_msg"] = "Failed to add RFID";
			header("Location:./addrfid.php");
		}
		else 
		{
			$_SESSION['success_msg'] = "RFID:$rfid_no was added to $operator_id.";
			$operator_id = $rfid_no = "";
			echo "<script> window.location.href = '".$_SERVER['REQUEST_URI']."'</script>";
		}
	}
...
{
	$match_operator_id = filter_input(INPUT_POST, "match_operator");
	$match_rfid = filter_input(INPUT_POST, "match_rfid");
	$match_user = Users::with_id($match_operator_id);
	...
	if($match_user->rfid_no != $match_rfid)
	{
		$_SESSION["error_msg"] = "No Match Found for ID:$match_operator with RFID:$match_rfid";
		header("Location:./addrfid.php");
	}
	else
	{
...
<div id="page-wrapper">
	...
	<div class="row">
		...
		<div class="col-md-5">
			<div class="panel panel-default">
				<div class="panel-heading">
					<a href="#" data-toggle="tooltip" data-placement="top" title="Below is the most recenly used RFID tag used on a power tail">
						<i class="fas fa-wifi fa-lg"> Recently Used RFID</i>
					</a>
				</div>
				<div class="panel-body">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<a href="#" data-toggle="tooltip" data-placement="top" title="Verify that a learner's RFID belongs to them, enter 2 of the 3 values.">
						<i class="fas fa-street-view fa-lg"> Read Match</i>
					</a>
				</div>
				<div class="panel-body">
					<form id="readForm" method="post" autocomplete="off" action="">
						<table class="table table-bordered table-striped">
							<tr>
								<td>ID Number</td>
								<td>
									<input type="text" name="match_operator" id="match_operator" placeholder="1000000000" 
											value="<?php if(isset($match_operator_id)) echo $match_operator_id;?>" maxlength="10" 
											size="10" autofocus tabindex="10" class='form-control'>
								</td>
							</tr>
							<tr>
								<td>RFID (HEX)</td>
								<td>
									<input type="text" name="match_hex" id="match_hex" placeholder="RFID HEX Value" 
							...
							<tr>
								<td>RFID (Decimal)</td>
								<td>
									<input type="text" name="match_rfid" id="match_rfid" placeholder="RFID Number" 
							...
							<tr>
								<td colspan="2">
									<button class="btn btn-basic pull-right" tabindex="13" name="matchBtn">Match Me</button>
								</td>
							</tr>
						</table>
					</form>
