...
function RFIDtoUTAID ($rfid_no) {
    ...
    ")){
        ...
        ErrorLog::insertError($_SERVER['PHP_SELF'],  $mysqli->error, "");
    }
...
function end_transaction( $d_id ){
    ...
    if (! (preg_match("/^\d*$/", $d_id))) {
        ...
        ErrorLog::insertError($_SERVER['PHP_SELF'],  $json_out["ERROR"], "");
        ErrorExit(1);
    }
    ...
    if ($msg === true){
        ...
        $json_out["success"] = "N";
        ErrorLog::insertError($_SERVER['PHP_SELF'],  $msg, "");
        ErrorExit(1);
    }
}
...
function ErrorExit ($exit_status) {
    ...
    if(isset($input_data["device"])){
        ...
        ErrorLog::insertError("JuiceBox: ".$device->getDevice_desc(), $json_out['ERROR'], "");
    } else {
        ErrorLog::insertError("JuiceBox", $json_out['ERROR'], "");
    }
    echo json_encode($json_out);
    $mysqli->close();
    exit();
