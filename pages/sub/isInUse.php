...
if (!empty($_GET["dg_id"]) || !empty($_GET["operator"]) && DeviceGroup::regexDgID($_GET["dg_id"]) === true && Users::regexUser($_GET["operator"])) {
    $dg = new DeviceGroup($_GET["dg_id"]);
    if ($dg->getGranular_wait() == 'N'){
        ...
        if ($result = $mysqli->query("
            ...
            WHERE public_view = 'Y' AND t.`trans_id` IS NOT NULL AND `devices`.`dg_id` = ".$dg->getDg_id()." AND `operator` = '$_GET[operator]'
