...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['srBtn'])){
    ...
    if (is_string($msg)){
        ...
        header("Location:sr_history.php?d_id=".$devices->getD_id());
    }
}
?>
                                        ...
                                        <select class="form-control" name="devGrp" id="devGrp" onChange="change_group()" >
                                            ...
                                            <?php if($result = DeviceGroup::popDGs()){
                                                while($row = $result->fetch_assoc()){
                                                    if (is_object($device) && $device->getDg()->getDg_id() == $row['dg_id']) {
                                                        echo("<option value='$row[dg_id]' selected>$row[dg_desc]</option>");
                                                    } else {
                                        ...
                                        <div class="input-group">
                                            <span class="input-group-addon" id="dot_span">
                                                <?php if (is_object($device)){
                                                    Devices::printDot($staff, $device->getD_id());
                                                } else { ?>
                                            ...
                                            <select class="form-control" name="devices" id="devices" tabindex="2" onchange="change_dot()">
                                                <?php if (is_object($device)){
                                                    echo ("<option value='".$device->getD_id()."'>".$device->getDevice_desc()."</option>");
                                                } else { ?>
